...
gpu_matrix.prototype = {
	...
	toString: function (radix) {
		if (this.data === undefined) return "undefined gpu_matrix";
		var radix = radix || 10;
		var s = "[";
		var idx = 0,
			n = this.data.length;
	...
	setData: function (r, c, data) {
		...
		if (
			!(data instanceof Array || data.subarray) ||
			r === undefined ||
			c === undefined ||
	...
	_checkinit: function (r, c) {
		...
		if (this.__gpumatrix__ === undefined) {
			...
			// get webgl context
			gpu_matrix.prototype.__gpumatrix__ = canvas.getContext(
				"experimental-webgl",
			...
			try {
				ext =
					gpu_matrix.prototype.__gpumatrix__.getExtension("OES_texture_float");
			} catch (e) {}
			if (!ext) {
	...
	_buildRenderer: function (gl) {
		...
		var vertShader = this._getShader(
			gl,
			"x-shader/x-vertex",
			this._vertexShader
		);
		if (this.supportsOES_TEXTURE_FLOAT_EXTENSION) {
			var fragShader = this._getShader(
				gl,
				"x-shader/x-fragment",
				this._shader_FLOAT
			);
